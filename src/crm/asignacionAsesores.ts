 ...
 * si el bot vuelve a escalar la misma conversación (reconexión, nuevo mensaje tras escalar) no
 * reasigna ni duplica la tarea. No hace nada si el deal no es de uno de los 2 programas piloto, si
 * faltan los IDs de asesor en la config, o si no hay Redis (no hay forma confiable de alternar sin
 * estado compartido entre réplicas).
 */
export async function asignarAsesorPorTurno(entities: CrmEntities, auth: Auth): Promise<void> {
  ...
  try {
    ...
    const t = await callCrm<BitrixTaskAddResult>(
      ...
      {
        fields: {
          TITLE: `📞 Seguimiento (${prog.nombre}) — Deal #${entities.deal}`,
          DESCRIPTION:
